...
Qibo features a model implementing double-bracket quantum algorithms (DBQAs) which are helpful for approximating eigenstates based on the ability to run the evolution under the input Hamiltonian.
...
DBQAs go beyond VQE by using a different approach to compile quantum circuits that approximate the eigenstates [3].
...
This equation shows that if the scalar product $\langle W, [H,\sigma(H)\rangle$ is positive, then after applying the DBR, the magnitude of the off-diagonal couplings in $H_0(s)$ decreases compared to $H_0$.
